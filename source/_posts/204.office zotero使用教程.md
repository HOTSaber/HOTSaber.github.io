...
需要更新`Translators`,在Zotero的`设置`中，找到`Jasminum`选项(需要安装`Jasminum`插件)下的`更新翻译器`。点击`更新全部`按钮，确保所有的`Translators`（特别是针对知网的CNKI translator）都是最新版本。这对于确保Zotero能够准确抓取到网页信息至关重要。
### zotero无法与word联接
通常在插入引文时报此错误
